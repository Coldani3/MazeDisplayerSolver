...
//
//NeuralNetworkSolver::NeuralNetworkSolver(Maze maze, int maxNetworkGroups, int networksPerGroup, int maxLayersPerNetwork, int trainsPerCycle) : Solver(maze) {
...
//	setupNetworks();
//
//	std::vector<int> startMazePos = maze.mazeEntrance;
//
...
//		int directionsAvailable = maze[positionsOfNetworks[train]];
//
//		for (int i = 0; i < startMazePos.size(); i++) {
//			distances.push_back(positionsOfNetworks[train][i]);
//			distances.push_back(maze.sizes[i] - positionsOfNetworks[train][i]);
//		}
//
...
//		distance += pow(abs(maze.mazeExit[i] - finalCoords[i]), 2);
//	}
//
